...
SELECT
    ...
    , DATE_ADD(loi.order_date_time, interval -1 MONTHS) old_date_time
    , loi.user_id user_id
...
column_name = ['date_time','old_date_time', 'user_id', 'point_amt', 'month', 'hour', 'weekday', 'age', 'gender', 'category']
...
#count = pd.DataFrame()
count = []
for userId, dateTime, oldDateTime in zip(df['user_id'],df['date_time'],df['old_date_time']):
#  print(userId, dateTime, oldDateTime)
  temp = df[df['user_id'] == userId]
  temp = temp[temp['date_time'] >= oldDateTime]
  temp = temp[temp['date_time'] <= dateTime]
...
df.drop('old_date_time', axis=1, inplace=True)
...
try:
    model.save("model_autokeras_category", save_format="tf")
except Exception:
    model.save("model_autokeras.h5")
    
model.summary()
loaded_model = tf.keras.models.load_model("model_autokeras_category", custom_objects=ak.CUSTOM_OBJECTS)
